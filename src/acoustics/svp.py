...
def DelGrosso_SV(S, T, P):
    ...
    const = {
        ...
        "C_ST": -0.1275936 * 10**-1,
        "C_TP": 0.6353509 * 10**-2,
        "C_T2P2": 0.2656174 * 10**-7,
        "C_TP2": -0.1593895 * 10**-5,
        "C_TP3": 0.5222483 * 10**-9,
        "C_T3P": -0.4383615 * 10**-6,
        "C_S2P2": -0.1616745 * 10**-8,
        "C_ST2": 0.9688441 * 10**-4,
        "C_S2TP": 0.4857614 * 10**-5,
        "C_STP": -0.3406824 * 10**-3,
    }
    ...
    Delta_CSTP = (
        ...
        + const["C_T3P"] * P * T**3
        + const["C_TP2"] * T * P**2
        + const["C_T2P2"] * T**2 * P**2
        + const["C_TP3"] * T * P**3
        + const["C_ST"] * S * T
        + const["C_ST2"] * S * T**2
        + const["C_STP"] * S * T * P
        + const["C_S2TP"] * S**2 * T * P
        + const["C_S2P2"] * S**2 * P**2
    )
...
def UNESCO_SV(S, T, P):
    ...
    D = {"00": 1.727 * 10**-3, "10": -7.9836 * 10**-6}
    ...
    Cw_TP = (
        ...
        + (C["20"] + C["21"] * T + C["22"] * T**2 + C["23"] * T**3 + C["24"] * T**4)
        * P**2
