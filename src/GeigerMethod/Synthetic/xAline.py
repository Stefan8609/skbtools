...
def index_data(offset, CDOG_data, GPS_data, travel_times, transponder_coordinates, esv):
    ...
    CDOG_mask = (CDOG_match < len(full_times)) & (
        full_times[CDOG_match] == unique_CDOG_times
    )
    ...
    GPS_mask = (GPS_match < len(full_times)) & (
        full_times[GPS_match] == unique_GPS_times
    )
...
def find_subint_offset(
    ...
    for interval in intervals:
        for lag in np.arange(lower, upper + interval, interval):
            ...
            lag = np.round(lag, 10)
            CDOG_full, GPS_full = index_data(
                lag, CDOG_data, GPS_data, travel_times, transponder_coordinates, esv
            )[1:3]
...
def two_pointer_index(
    ...
    while CDOG_pointer < len(CDOG_data) and GPS_pointer < len(GPS_data):
        if np.abs(GPS_times[GPS_pointer] - CDOG_times[CDOG_pointer]) < threshhold:
            ...
            transponder_coordinates_full = np.vstack(
                (transponder_coordinates_full, transponder_coordinates[GPS_pointer])
            )
...
if __name__ == "__main__":
    ...
    full_times, CDOG_full, GPS_full, transponder_full, esv_full = index_data(
        1200, CDOG_data, GPS_data, travel_times, transponder_coordinates, esv
    )
    ...
    ax1.scatter(
        full_times, GPS_full, s=1, marker="o", label="Calculated GPS Travel Times"
    )
