...
def fourPlot(n, time_noise, position_noise):
    ...
    axes[0, 0].text(
        0.5,
        0.5,
        "No Resulting Noise in Estimation",
    ...
    axes[0, 1].hist(
        diff_data2 * 1000, orientation="vertical", bins=30, alpha=0.5, density=True
    )
    ...
    std_height, noise_height = (
        norm.pdf(std2, mu2, std2),
        norm.pdf(time_noise * 1000, 0, std2),
    )
    axes[0, 1].arrow(
        mu2,
        std_height,
    ...
    axes[0, 1].vlines(
        [mu2 - std2, mu2 + std2, -time_noise * 1000, time_noise * 1000],
        [0, 0, 0, 0],
        [std_height, std_height, noise_height, noise_height],
        colors=["k", "k", "r", "r"],
        linestyles="dashed",
    )
    ...
    axes[1, 0].hist(
        diff_data3 * 1000, orientation="vertical", bins=30, alpha=0.5, density=True
    )
    ...
    std_height, noise_height = (
        norm.pdf(std3, mu3, std3),
        norm.pdf(position_noise / 1515 * 1000, 0, std3),
    )
    axes[1, 0].arrow(
        mu3,
        std_height,
    ...
    axes[1, 1].hist(
        diff_data4 * 1000, orientation="vertical", bins=30, alpha=0.5, density=True
    )
    ...
    std_height, time_noise_height, position_noise_height = (
        norm.pdf(std4, mu4, std4),
        norm.pdf(time_noise * 1000, 0, std4),
