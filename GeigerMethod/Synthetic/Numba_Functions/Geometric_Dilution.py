...
def geometric_dilution(interval):
    ...
    inversion_result, CDOG_data, CDOG_full, GPS_data, GPS_full, CDOG_clock, GPS_clock, transponder_coordinates, GPS_Coordinates, offset = (
        modular_synthetic(2 * 10**-5, 2 * 10**-2, 0, 0, "global_table_esv",table_str, generate_type = 1, inversion_type=0, plot=False)
    )
    ...
    times, esv = calculateTimesRayTracing_Bias(CDOG_guess, transponder_coordinates, esv_bias, dz_array, angle_array, esv_matrix)
    ...
    for i in range(0, len(transponder_coordinates)-interval, interval):
        ...
        J = compute_Jacobian_biased(CDOG_guess, coords_inter, times_inter, esv_inter, esv_bias)
