...
def compare_tables(esv_table1, esv_table2):
    ...
    estimate, times_known = numba_bias_geiger(CDOG_guess, CDOG, transponder_coordinates_Actual,
                                              transponder_coordinates_Found, esv_bias, time_bias, dz_array_inv, angle_array_inv, esv_matrix_inv,
                                              dz_array_gen, angle_array_gen, esv_matrix_gen, time_noise)
    ...
    print(f"Distance: {np.sqrt(np.sum((estimate[:3] - np.array([CDOG[0], CDOG[1], CDOG[2]])))**2)}")
    ...
    print("Average Difference in ESV table at Relevant Angles and Depths: ", tot / len(gen_esv), "\nESV Bias: ", estimate[4])
