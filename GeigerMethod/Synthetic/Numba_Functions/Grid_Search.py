        ...
        for x in x_grid:
            for y in y_grid:
                for z in z_grid:
                    ...
                    best_lever, best_offset, inversion_result = simulated_annealing_bias(300, CDOG_data, GPS_data,
                                                                                         GPS_Coordinates, gps1_to_others,
                                                                                         CDOG_guess, initial_lever,
                    ...
                    inversion_result, CDOG_full, GPS_full, CDOG_clock, GPS_clock = final_bias_geiger(inversion_guess,
                                                                                                     CDOG_data, GPS_data,
                                                                                                     transponder_coordinates,
                                                                                                     best_offset, esv_bias,
                                                                                                     time_bias,
                    ...
                    file.write("[{} ], [{} ], {:.4f}, {:.4e}, {:.4f}, {:.4f}\n".format(
                        np.array2string(np.round(best_lever, 4), separator=', ')[1:-1],
                        np.array2string(np.round(inversion_guess, 4), separator=', ')[1:-1],
                        best_offset, time_bias, esv_bias, RMSE))
                    iteration += 1
