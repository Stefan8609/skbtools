...
@njit
def _evaluate(CDOG_data, GPS_data, GPS_Coordinates, gps1_to_others, inversion_in, lever,
                             dz_array, angle_array, esv_matrix, offset):
    ...
    inversion_result, CDOG_full, GPS_full, CDOG_clock, GPS_clock = final_bias_geiger(inversion_guess, CDOG_data,
                                                                                     GPS_data,
                                                                                     transponder_coordinates, offset,
                                                                                     esv_bias, time_bias,
                                                                                     dz_array, angle_array, esv_matrix, True)
...
@njit
def simulated_annealing_real(max_iterations, CDOG_data, GPS_data, GPS_Coordinates, gps1_to_others, initial_guess, initial_lever,
                             dz_array, angle_array, esv_matrix, offset=2000.0):
    """Simulated Annealing Algorithm
        Optimized for real data with an inputted offset...
    """
    inversion_estimate = np.array([initial_guess[0], initial_guess[1], initial_guess[2], 0.0, 0.0])
    best_rmse = np.inf
    ...
    while k < max_iterations:
        ...
        # Calculate the RMSE
        inversion_estimate, RMSE = _evaluate(CDOG_data, GPS_data, GPS_Coordinates, gps1_to_others, inversion_estimate, lever,
                         dz_array, angle_array, esv_matrix, offset)
        if RMSE < best_rmse:
            best_rmse = RMSE
...
if __name__ == "__main__":
    ...
    initial_transponder_coordinates = findTransponder(GPS_Coordinates, gps1_to_others, initial_lever)
    ...
    for off_adj in np.linspace(-3, 3, 7):
        inversion_result, best_lever, RMSE = simulated_annealing_real(300, CDOG_data, GPS_data, GPS_Coordinates, gps1_to_others, initial_guess, initial_lever,
                             dz_array, angle_array, esv_matrix, offset=offset + off_adj)
        ...
        print("Distance: {:.2f} cm".format(np.linalg.norm(inversion_guess - CDOG) * 100), "RMSE: ", np.round(RMSE * 100 * 1515, 3))
        print("\n")
