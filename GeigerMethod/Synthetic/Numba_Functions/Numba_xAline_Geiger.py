...
from Numba_Geiger import computeJacobianRayTracing, findTransponder, calculateTimesRayTracing, calculateTimesRayTracingReal
...
def initial_geiger(guess, CDOG_data, GPS_data, transponder_coordinates, real_data=False):
...
def final_geiger(guess, CDOG_data, GPS_data, transponder_coordinates, offset, real_data=False):
    ...
    while np.linalg.norm(delta) > epsilon and k < 10:
        if real_data == False:
            ...
            GPS_full, esv = calculateTimesRayTracingReal(inversion_guess, transponder_coordinates_full)
