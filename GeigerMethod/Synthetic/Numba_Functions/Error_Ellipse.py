...
def _plot_error_ellipse(ax, x, y, n_std=1.0, facecolor='none', edgecolor='black',
                       linestyle='--', label=None, **ellipse_kwargs):
    """
...
def error_ellipse(num_points, time_noise, position_noise):
    ...
    for i in range(num_points):
        (inversion_result, CDOG_data, CDOG_full, GPS_data, GPS_full, CDOG_clock, GPS_clock, CDOG, transponder_coordinates,
         GPS_Coordinates, offset, lever) = modular_synthetic(time_noise, position_noise, 0, 0,
    ...
    # Easting vs Northing
    sc0 = axs[0].scatter(estimate_converted[:, 0] * 100, estimate_converted[:, 1] * 100, c=RMSE_array, cmap="viridis",
                         s=5, label=r'CDOG Estimates')
    axs[0].scatter(0, 0, s=100, color="red", marker="o", label="CDOG Actual")
    for std in range(4):
        _plot_error_ellipse(axs[0], estimate_converted[:, 0]*100, estimate_converted[:, 1]*100, n_std=std,
                            facecolor='none', edgecolor='black', linestyle='--', label=None)
    ...
    sc1 = axs[1].scatter(estimate_converted[:, 0] * 100, estimate_converted[:, 2] * 100, c=RMSE_array, cmap="viridis",
                         s=5, label=r'CDOG Estimates')
    axs[1].scatter(0, 0, s=100, color="red", marker="o", label="CDOG Actual")
    for std in range(2):
        _plot_error_ellipse(axs[1], estimate_converted[:, 0]*100, estimate_converted[:, 2]*100, n_std=std,
                            facecolor='none', edgecolor='black', linestyle='--', label=None)
    ...
    sc2 = axs[2].scatter(estimate_converted[:, 1] * 100, estimate_converted[:, 2] * 100, c=RMSE_array, cmap="viridis",
                         s=5, label=r'CDOG Estimates')
    cbar2 = fig.colorbar(sc2, ax=axs[2])
    cbar2.set_label("RMSE (cm)")
    axs[2].scatter(0, 0, s=100, color="red", marker="o", label="CDOG Actual")
    for std in range(2):
        _plot_error_ellipse(axs[2], estimate_converted[:, 1]*100, estimate_converted[:, 2]*100, n_std=std,
                            facecolor='none', edgecolor='black', linestyle='--', label=None)
    ...
    fig, axs = plt.subplots(1, 3, figsize=(15, 5))
    # x vs y error
    sc3 = axs[0].scatter(lever_diff_array[:, 0] * 100, lever_diff_array[:, 1] * 100, c=RMSE_array, cmap="viridis",
                         s=5, label=r'Lever Errors')
    for std in range(4):
        _plot_error_ellipse(axs[0], lever_diff_array[:, 0]*100, lever_diff_array[:, 1]*100, n_std=std,
                            facecolor='none', edgecolor='black', linestyle='--', label=None)
    ...
    # x vs z error
    sc4 = axs[1].scatter(lever_diff_array[:, 0] * 100, lever_diff_array[:, 2] * 100, c=RMSE_array, cmap="viridis",
                         s=5, label=r'Lever Errors')
    for std in range(2):
        _plot_error_ellipse(axs[1], lever_diff_array[:, 0] * 100, lever_diff_array[:, 2] * 100, n_std=std,
                            facecolor='none', edgecolor='black', linestyle='--', label=None)
    ...
    # y vs z error
    sc5 = axs[2].scatter(lever_diff_array[:, 1] * 100, lever_diff_array[:, 2] * 100, c=RMSE_array, cmap="viridis",
                         s=5, label=r'Lever Errors')
    cbar5 = fig.colorbar(sc5, ax=axs[2])
    for std in range(2):
        _plot_error_ellipse(axs[2], lever_diff_array[:, 1] * 100, lever_diff_array[:, 2] * 100, n_std=std,
                            facecolor='none', edgecolor='black', linestyle='--', label=None)
