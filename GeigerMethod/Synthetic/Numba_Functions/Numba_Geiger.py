...
def generateRealistic(n):
    ...
    for i in range(n//4 * 4):
        ...
        xRot = np.array(
            [[1.0, 0.0, 0.0], [0.0, np.cos(rot[i, 0]), -np.sin(rot[i, 0])], [0.0, np.sin(rot[i, 0]), np.cos(rot[i, 0])]])
        yRot = np.array(
            [[np.cos(rot[i, 1]), 0.0, np.sin(rot[i, 1])], [0.0, 1.0, 0.0], [-np.sin(rot[i, 1]), 0.0, np.cos(rot[i, 1])]])
        zRot = np.array(
            [[np.cos(rot[i, 2]), -np.sin(rot[i, 2]), 0.0], [np.sin(rot[i, 2]), np.cos(rot[i, 2]), 0.0], [0.0, 0.0, 1.0]])
        ...
        transponder_coordinates[i] = GPS_Coordinates[i, 0] + totalRot @ gps1_to_transponder
...
if __name__ == "__main__":
    ...
    guess = np.array([random.uniform(-5000, 5000), random.uniform(-5000, 5000), random.uniform(-5225, -5235)])
