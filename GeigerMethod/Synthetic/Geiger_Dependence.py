...
def point_dependence(time_noise, position_noise):
    ...
    plt.axhline(
        time_noise + position_noise / 1515, color="b", label="Time and Position Noise"
    )
...
def combined_dependence(n):
    ...
    for i in range(len(X[0])):
        ...
        for j in range(len(Y[:, 0])):
            ...
            GPS_Coordinates += np.random.normal(
                0, X[0, i], (len(GPS_Coordinates), 4, 3)
            )
            transponder_coordinates_Found = findTransponder(
                GPS_Coordinates, gps1_to_others, gps1_to_transponder
            )

            guess, times_known, est = geigersMethod(
                initial_guess,
                CDog,
                transponder_coordinates_Actual,
                transponder_coordinates_Found,
    ...
    CS = plt.contour(
        X * 100, Y * 1000, Z_exp * 1515 * 100, colors="k", levels=np.arange(0, 50, 5)
    )
...
def random_dependence(points, n):
    ...
    for i in range(points):
        ...
        GPS_Coordinates += np.random.normal(
            0, position_noise[i], (len(GPS_Coordinates), 4, 3)
        )
        transponder_coordinates_Found = findTransponder(
            GPS_Coordinates, gps1_to_others, gps1_to_transponder
        )
    ...
    plt.scatter(
        time_noise * 1515 * 100, position_noise * 100, c=(std_observed / std_expected)
    )
    cbar = plt.colorbar()
