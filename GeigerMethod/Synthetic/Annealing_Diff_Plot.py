...
def annealing_plot(n, time_noise, position_noise, lever_noise=5, geom_noise=0):
    ...
    offset_times = calculateTimesRayTracing(
        offset_estimate, transponder_coordinates_Found
    )[0]
    ...
    transponder_coordinates_annealing = findTransponder(
        GPS_Coordinates, gps1_to_others, lever
    )
    annealing_estimate = geigersMethod(
        annealing_guess,
        CDog,
        transponder_coordinates_Actual,
        transponder_coordinates_annealing,
        time_noise,
    )[0]
    annealing_times = calculateTimesRayTracing(
        annealing_estimate, transponder_coordinates_annealing
    )[0]
    ...
    axs[0, 1].scatter(
        ...
        label="Transducer Trajectory",
        color="b",
        marker="o",
        s=5,
    )
    axs[0, 1].scatter(
        CDog[0], CDog[1], label="CDOG Location", color="k", marker="x", s=20
    )
