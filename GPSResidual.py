...
for i in range(len(xyz_array)):
    ...
    barycenter = np.mean(np.array([xyz_array[i][0],xyz_array[i][1],xyz_array[i][2]]), axis=1)
    for p in range(4):
        pointVect = np.array([xyz_array[i][0][p],xyz_array[i][1][p],xyz_array[i][2][p]]) - barycenter
