...
schematic_grid = np.array([[0.0, 0.0, 0.0], [0.0, -4.4, 0.060621], [-10.9, -6.5, 0.00877], [-10.9, 0.4, 0.04880436]])
...
# Plot the GPS points
ax.scatter(gps1_to_others[:, 0], gps1_to_others[:, 1], gps1_to_others[:, 2],
           color='b', marker='o', s=50, label='GPS Points from averaging distance')
ax.scatter(schematic_grid[:, 0], schematic_grid[:, 1], schematic_grid[:, 2],
           color='r', marker='o', s=50, label='GPS Points from schematic')
...
ax.scatter(rotated_schematic_grid[:, 0], rotated_schematic_grid[:, 1], rotated_schematic_grid[:, 2],
           color='g', marker='o', s=50, label='Rotated Schematic Points')

# Plot the quiver arrow from arrow_start in the direction of arrow_vector
ax.quiver(0, 0, 0,
          pf_to_transducer[0], pf_to_transducer[1], pf_to_transducer[2],
          color='r', label='Schematic Transducer')
ax.quiver(0, 0, 0,
          gps1_to_transducer[0], gps1_to_transducer[1], gps1_to_transducer[2],
          color='b', label='Lever Arm Estimate')
...
ax.quiver(pf_to_transducer[0], pf_to_transducer[1], pf_to_transducer[2],
          diff_arrow[0], diff_arrow[1], diff_arrow[2],
          color='g', label='Rotated Transducer')
